...
- Added a password-style fxRatesAPI key field with help text explaining the more frequently updated feed.
- Added Android Keystore-backed AES-GCM encryption for the locally stored fxRatesAPI key.
- Added provider-scoped rate caching and backup exclusions for encrypted credentials.
- Added QA and production build gates with versioned APK artifact names.
...
- Updated the widget status text to distinguish the selected provider's refresh behavior.

### Fixed

- Fixed fxRatesAPI key saving on Android Keystore implementations that reject caller-supplied initialization vectors.
- Added recovery when the Android Keystore encryption key becomes permanently invalidated.
- Prevented stale in-flight responses from being written after provider or API-key changes.
