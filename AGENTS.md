...
5. During QA/local APK builds, keep the current feature `versionName` and bump only `versionCode`; never bump a patch or hotfix version until a release-tag push is explicitly requested. GitHub releases must use only the production artifact from `release.sh` with an exact `x.y.z` version.
