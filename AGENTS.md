...
5. Use the old build workflow: local/testing/QA APKs use the debug certificate, preserve the latest released `versionName`, and bump only `versionCode`; the QA build script rejects stale version names. Never append a QA suffix or assume a patch/hotfix version. Production releases use `release.sh` with the production certificate and an exact `x.y.z` version. Change the patch or hotfix version only when an explicit release-tag push is requested. GitHub releases must use only the verified production artifact.
